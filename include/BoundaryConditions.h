...
void update_bcs(Grid& grid, const SimulationConfig& cfg, Array2D<T>& array){
    ...
    switch(cfg.bcs_x0){
        ...
        for(size_t j=0;j<(ny+2*g);j++){
            ...
            //Normal velocity is mirrored and negated i.e. no normal flux;
            if constexpr (std::is_same_v<T, ConservedStateVector>) {
            array(0,j).momentum().x()*=-1;
            array(1,j).momentum().x()*=-1;
            //Normal B field is set to initial value (in this case 0)
            array(0,j).B().x()*=-1;
            array(1,j).B().x()*=-1;}
            else if constexpr (std::is_same_v<T, PrimitiveStateVector>){
            array(0,j).velocity().x()*=-1;
            ...
            array(0,j).B().x()*=-1;
            array(1,j).B().x()*=-1;}
            else if constexpr (std::is_same_v<T, Vector3 >){//Case where grid.B is updated
            array(0,j).x()*=-1;
            array(1,j).x()*=-1;   
                }
        }
    ...
    switch (cfg.bcs_xf) {
        ...
        case BCs::Conducting:
            for (size_t j = 0; j < ny + 2*g; j++) {
                ...
                if constexpr (std::is_same_v<T, ConservedStateVector>) {
                //Reflect the normal velocity/momentum
                array(nx+g,j).momentum().x()*=-1;
                array(nx+g+1,j).momentum().x()*=-1;
                //For initial 0 B field normal component stays constant
                array(nx+g,j).B().x()*=-1;
                array(nx+g+1,j).B().x()*=-1;}
                else if constexpr (std::is_same_v<T, PrimitiveStateVector>){
                array(nx+g,j).velocity().x()*=-1;
    ...
    switch(cfg.bcs_y0){
        ...
        for(size_t i=0;i<(nx+2*g);i++){
            ...
            array(i,0).momentum().y()*=-1;
            array(i,1).momentum().y()*=-1;
            //For initial normal B field of 0
            array(i,0).B().y()*=-1;
            array(i,1).B().y()*=-1;}
            else if constexpr (std::is_same_v<T, PrimitiveStateVector>){
            array(i,0).velocity().y()*=-1;
